...
def profile_view(request):    
    ...
    context = {
        ...
        'account_act': "vMenu--active",
        'dash_style': "opacity:1;border-bottom: 1px solid #fff;"
    }
...
@login_required
def update_transactions_view(request, subscription_id):
    user_membership = get_user_membership(request)
    selected_membership = get_selected_membership(request)
...
@login_required
def confirm_cancel_view(request):
    return render(request, 'confirm_cancel.html', {})
...
def cancel_subscription_view(request):
    ...
    messages.info(
        request, "You have successfully cancelled your membership.")
    # send an email here

    return redirect(reverse('cancel'))